...
可以从 `amount` 开始，每次从 `coins` 的硬币中选中 `1` 枚硬币，并记录当前挑选硬币的次数，则最快减到 `0` 的次数就是凑成总金额所需的最少的硬币个数。这道题就变成了从 `amount` 减到 `0` 的最短路径问题，因此可以用 `BFS` 的方法来做s
    ...
    def coinChange(self, coins: List[int], amount: int) -> int:
        ...
        while que:
            ...
            for i in range(size):
                ...
                # 尝试用每个硬币面值来减少当前金额
                for coin in coins:
                    # 如果当前状态等于某个硬币面值，说明当前步数就能凑成该硬币面值
                    if cur == coin:
                        return step
                    
                    # 如果当前状态 cur 减去硬币面值 coin 后依然大于 0
...
class Solution {
    public int coinChange(int[] coins, int amount) {
        ...
        // 如果 dp[amount] 还是无穷大，表示无法凑成目标金额，返回 -1
    ...
    def coinChange(self, coins: List[int], amount: int) -> int:
        ...
        for coin in coins:
            ...
            for i in range(coin, amount + 1):
                # 更新凑成金额 i 所需的最少硬币数量
                # dp[i - coin] + 1 表示如果可以用 dp[i - coin] 的方案再加上一个 coin 硬币，就可以凑成 i
...
var coinChange = function(coins, amount) {
    ...
    // 如果 dp[amount] 还是 Infinity，表示无法凑成目标金额，返回 -1
...
class Solution {
    public int coinChange(int[] coins, int amount) {
        // 初始化 dp 数组，大小为 amount + 1，所有值初始化为无穷大（表示无法凑成该金额）
        int[] dp = new int[amount + 1];
        Arrays.fill(dp, Integer.MAX_VALUE);
...
class Solution:
    def coinChange(self, coins: List[int], amount: int) -> int:
        ...
        for i in range(1, amount + 1):  
            for coin in coins:
                # 当前硬币 coin 可以凑成当前金额 i（即 i - coin >= 0），则比较 dp[i] 和 dp[i - coin] + 1 取最小值
                if i - coin >= 0:
                    # 更新凑成金额 i 所需的最少硬币数量
                    # dp[i - coin] + 1 表示如果可以用 dp[i - coin] 的方案再加上一个 coin 硬币，就可以凑成 i
                    dp[i] = min(dp[i], dp[i - coin] + 1)
