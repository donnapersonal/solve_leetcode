...
class Solution {
    public int longestSubarray(int[] nums, int limit) {
        PriorityQueue<int[]> heapMax = new PriorityQueue<>((a, b) -> b[0] - a[0]);
        PriorityQueue<int[]> heapMin = new PriorityQueue<>(Comparator.comparingInt(a -> a[0]));
        int res = 0;
        int left = 0, right = 0;
        while (right < nums.length) {
            heapMax.offer(new int[]{nums[right], right});
            heapMin.offer(new int[]{nums[right], right});
            while (heapMax.peek()[0] - heapMin.peek()[0] > limit) {
                while (heapMin.peek()[1] <= left) {
                    heapMin.poll();
                }
                while (heapMax.peek()[1] <= left) {
                    heapMax.poll();
                }
                left++;
            }
