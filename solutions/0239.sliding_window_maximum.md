...
遍历一遍的过程中每次从窗口中再找到最大的数值，这样很明显是 `O(n × k)` 的算法
...
class Solution {
    public int[] maxSlidingWindow(int[] nums, int k) {
        ...
        for(int i = 0; i < k; i++) {
            ...
            deque.addLast(i);  // 将当前元素的索引加入到队列中
        }
        // 队列头部元素是前 k 个元素的最大值
        res[num++] = nums[deque.getFirst()];
        for(int i = k; i < nums.length; i++) {
            if(!deque.isEmpty() && deque.getFirst() <= i - k) {
                deque.removeFirst();
            }
            // 向队列中添加元素时，移除所有比当前元素小的值
            while(!deque.isEmpty() && nums[deque.getLast()] <= nums[i]) {
                deque.removeLast();
            }
            deque.addLast(i);  // 将当前元素的索引加入到队列中
...
var maxSlidingWindow = function(nums, k) {
    ...
    for(let i = 0; i < nums.length; i++) {
        ...
        if(i >= k - 1) {
            res.push(nums[deque[0]]); // 队列头部元素为当前窗口的最大值
        }
    }
    return res;
};
